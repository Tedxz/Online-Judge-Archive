...
struct trie_node
{
    int val,max_val;
    trie_node *ch[26];
    trie_node():val(-1), max_val(-1) { memset(ch,0,sizeof(ch)); }
};
class trie
{
    private:
        ...
        void dfs(trie_node *p,string s)
        {
            if (p->val>0)
            {
                ...
                tem.key=s;
                tem.val=p->val;
                if ((list.size()<10) || cmp(tem,list[0]))
                {
                    list.push_back(tem);
            ...
            if ((list.size()>=10) && (p->max_val<list[0].val)) return;
            for (int i=25; i>=0; --i)
            {
        ...
        static inline bool cmp(word a, word b)
        {
            return (a.val>b.val)||((a.val==b.val)&&(a.key<b.key));
        }
    public:
        trie() { root=new trie_node; root->max_val=10000000; }
        void insert(word w)
        {
            trie_node *p=root;
            for (int i=0; i<w.key.size(); ++i)
            {
                ...
                p=p->ch[w.key[i]-'a'];
                p->max_val<w.val?p->max_val=w.val:0;
            }
            p->val=w.val;
        }
